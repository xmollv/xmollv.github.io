...
Now let's write a basic `UIViewController` that uses the `CoreDataNotifier` to drive it's data. It's out of the scope of this post to show [how to create](https://github.com/xmollv/Llitgi/blob/c61be8c6bcfb3f7a5fcba4c90cfde462d8c699b9/llitgi/DataProvider/CoreData/CoreDataFactory.swift#L46) an `NSFetchedResultsController`, but as you can see on the `init(fetchResultController:)`, you need one in order to instantiate this class. Make sure to replace the type used in the `CoreDataNotifier<NSManagedObject>` for the type that makes sense in your project.
...
After creating the above protocol with the default implementation, we can refactor our `ListViewController` to make it look like this:
