...
tags: [core-data]
---
In this post I wanted to share the way that I've been using `NSFetchedResultsController` in my Swift projects. The basic idea behind this approach is to wrap the `NSFetchedResultsController` and `NSFetchedResultsControllerDelegate` in a class that removes most of the boilerplate associated with it and makes it generic. Furthermore, using protocol extensions we can have or `UITableViewController` update it's data with just three lines of code.
...
Now let's write a basic `UIViewController` that uses the `CoreDataNotifier` to drive it's data. It's out of scope of this post top show how to create an `NSFetchedResultsController`, but as you can see on the `init(fetchedResultsController:)`, you need one in order to instantiate this class. Make sure to replace the type used in the `CoreDataNotifier<NSManagedObject>` to the type that makes sense in your project.
...
After creaing the above protocol with the default implementation, we can refactor our `ListViewController` to look like this:
